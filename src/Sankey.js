...
const highlightItem = (highlighted, target, on, primary = false) => {
  ...
  if (target.sourceLinks) {
    ...
    if (on) {
      ...
      highlighted.nodes = highlighted.nodes.filter(
        (n) => n.index !== target.index
      );
    }
  } else {
    //Link
    if (on) {
      ...
      highlighted.links = highlighted.links.filter(
        (n) => n.index !== target.index
      );
    }
  }
...
const Rect = ({
  ...
  const handleSelect = (e) => {
    ...
    if (data.type !== "agency" || agencyLevel < 3) {
      ...
      if (data.type === "agency")
        setAgencyLevel(agencyLevel >= 3 ? 3 : agencyLevel + 1);
    }
  };
  ...
  const labelLimit =
    (isMobile ? 10 : 20) - (data.type === "agency" ? 2 * agencyLevel : 0);

  label =
    label.length > labelLimit ? label.slice(0, labelLimit) + "..." : label;
  valueLabel =
    valueLabel.length > labelLimit
      ? valueLabel.slice(0, labelLimit) + "..."
      : valueLabel;
  trueValueLabel =
    trueValueLabel.length > labelLimit
      ? trueValueLabel.slice(0, labelLimit) + "..."
      : trueValueLabel;

  const isHighlighted =
    data.highlighted ||
    highlighted.nodes.find((n) => n.name === data.name && n.well === data.well);

  return (
    <>
      <g
        className={`node node-well-${data.well}${
          data.selected ? " selected" : ""
        }${isHighlighted ? " highlight" : ""} level-${
          data.type === "agency" ? agencyLevel : 0
        }`}
      ...
      >
        <rect
          ...
          strokeDasharray={
            y0 > 0
              ? `${
                  nodeWidth - 20 * (data.type === "agency" ? agencyLevel : 0)
                },${height * 2 + width}`
              : `0,${height * 2 + width * 2}`
          }
        />
        ...
        {!mini && data.type === "agency" && agencyLevel < 3 && well === 1 && (
          <FaChevronCircleLeft onClick={handleSelect} y={midY - 8} x={x0 + 2} />
        )}
      </g>
    </>
  );
...
const Link = ({ data, index, width, length, colors, textColors, mini }) => {
  ...
  const isHighlighted =
    data.highlighted ||
    highlighted.links.find(
      (l) =>
        l.source.name === data.source.name && l.target.name === data.target.name
    );
  ...
  return (
    <>
      <g
        className={`link ${isHighlighted ? "highlight" : ""}${
          data.selected ? " selected" : ""
        }`}
        onMouseMove={onMouseMove}
        onMouseOut={onMouseOut}
...
const Sankey = (props) => {
  ...
  const sankey = d3sankey()
    ...
    .extent([
      [0, 0],
      [totalActive == 2 && totalReady == 3 ? 630 : size.width, size.height],
    ]);

  const onMouseMove = (e) => {
    const bounds = e.target
      .closest(".component-sankey > svg")
  ...
  if (sankeyGraph && sankeyGraph.valid) {
    ...
    return (
      <div
        className={`component-sankey${isOver ? " drop-hover" : ""}${
          mini ? " mini" : ""
      ...
      >
                    ...
                    <React.Fragment key={i}>
                      {activeAgencyHierarchy.map((h, j) => {
                        if (diff[j] && d.agencyHierarchy[j]) {
                          const hierarchyNodes = nodes
                            .filter(
                              (n) =>
                                ...
                                n.agencyHierarchy[j] === d.agencyHierarchy[j]
                            )
                            .sort((a, b) => {
                          ...
                          const first = hierarchyNodes.find(
                            (n) =>
                              n.y0 ===
                              Math.min(...hierarchyNodes.map((n) => n.y0))
                          );
                          const last = hierarchyNodes.find(
                            (n) =>
                              n.y1 ===
                              Math.max(...hierarchyNodes.map((n) => n.y1))
                          );
                          const width = 20;
                          const x0 =
                            well === 0
                          ...
                          if (label && label.length > labelLimit - 1)
                            label =
                              label.slice(
                                0,
                                labelLimit > 1 ? labelLimit - 1 : 0
                              ) + (labelLimit > 1 ? "..." : "");
                          return (
                            <g
                              className="agency-grouping"
                              key={j}
                              index={j}
                            ...
                            >
                              ...
                              <text
                                x={x0 + width / 2}
                                y={y0 + height / 2}
                                transform={`rotate(90,${x0 + width / 2},${
                                  y0 + height / 2
                                })`}
                              >
                                {label}
        ...
        {!props.mini &&
          tooltip !== false &&
          highlighted.primary != undefined && <Tooltip opts={tooltip} />}
      </div>
    );
  }
