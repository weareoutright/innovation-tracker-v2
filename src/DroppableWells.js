...
const DroppableWell = (props) => {
  ...
  const [{ isOver, canDrop }, drop] = useDrop(
    () => ({
      ...
      canDrop: (item) =>
        (props.well === 0 || active[props.well - 1]) &&
        active.indexOf(item.text) === -1,
      drop: (item) => {
  ...
  return (
    <div
      ref={drop}
      className={`wells__well wells__well-${props.well}
