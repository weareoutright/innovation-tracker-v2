        ...
        <div className="well__headers">
          {active.map((well,w) => {
            return (<React.Fragment>
              <div className={`well__header${well !== null ? ' well__header-active' : ''}`} key={w}>
                {well !== null && 
                  <h3>
                    <span onClick={() => closeHeader(w)}>{labels.getLabel(well)} <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><path d="m12.45 37.65-2.1-2.1L21.9 24 10.35 12.45l2.1-2.1L24 21.9l11.55-11.55 2.1 2.1L26.1 24l11.55 11.55-2.1 2.1L24 26.1Z"/></svg></span>
                    <em>{selected[w] ? utils.getTruncatedWords(labels.getLabel(selected[w])) : 'All'}</em>
                  </h3>
                }
              </div>
              <React.Fragment>
                {w == 0 && anyActive && 
                  <div className="well__header well__header-active well__header-main">
                    <h2>{anySelected ? "Selected FY21 Climate Innovation Funding" : "FY21 Climate Innovation Funding"}
                      <em>{amount} ({percentage})</em>
                    </h2>
                  </div>
                }
              </React.Fragment>
