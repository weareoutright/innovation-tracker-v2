...
const organizeTooltipData = (summary) => {
  ...
  if (summaryList.length > summaryLimit) {
    const otherTotal = summaryList
      .slice(summaryLimit - 1)
      .reduce((a, b) => a + b[1], 0);
...
const initGroupedNode = (selected, active) => {
  ...
  if (selectedNodes.length) {
    let name = `${labels.getLabel(selectedNodes[0])}${
      selectedNodes[1] ? " & " + labels.getLabel(selectedNodes[1]) : ""
    }`;
    const node = {
      node: {
        name: name,
...
const Tray = (props) => {
  ...
  const getSummary = (node, type) => {
    ...
    const summary =
      !isNode || node[links].length === 0
        ? {}
        : node[links].reduce((obj, l) => {
  ...
  if (graph && graph.valid) {
    if (primary) {
      ...
      summaries = [];
      grouped =
        selected[0] || selected[1] ? initGroupedNode(selected, active) : null;
      graph.nodes.forEach((node) => {
        if (
          node.type === active[0] &&
          active[node.well] &&
          (!node.targetLinks.length ||
            node.targetLinks[0].source.name !== node.name)
        ) {
    ...
    summaries && (
      <aside className="component-tray">
        ...
        <div className="tray__content">
          ...
          {(selected[0] || selected[1]) && <Sankey mini={true} />}
          {!primary &&
